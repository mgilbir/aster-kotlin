...
216 Vega differential fixtures and 324 Vega-Lite fixtures pass, every one of them matching upstream
...
| Diagnostics, canonical snapshots, goldens, oracle scaffolding | No upstream equivalent. Two differential oracles, one for Vega and one for Vega-Lite, with 216 Vega differential fixtures and 324 Vega-Lite fixtures |
...
comparison on the stray property, and the scene comparison on labels drawn at full length. 324
...
**empty by default** and the emitted specification is byte-for-byte what it was, which is what the 324
