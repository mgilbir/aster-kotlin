...
213 Vega differential fixtures and 290 Vega-Lite fixtures pass, every one of them matching upstream
...
| Diagnostics, canonical snapshots, goldens, oracle scaffolding | No upstream equivalent. Two differential oracles, one for Vega and one for Vega-Lite, with 213 Vega differential fixtures and 290 Vega-Lite fixtures |
...
comparison on the stray property, and the scene comparison on labels drawn at full length. 290
...
**empty by default** and the emitted specification is byte-for-byte what it was, which is what the 290
