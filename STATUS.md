...
| `vega-transforms` (24 of 40) | 3,754 | the 12 the brief lists plus `timeunit`, `pie`, `window`, `sequence`, `lookup`, `impute`, `cross`, `pivot`, `countpattern`, `quantile`, `regression` and `loess`, exact against upstream |
...
| 9. At least 100 compatibility fixtures pass | 52 of 100 |
...
- 1,120 JVM tests pass (`./scripts/test-core.sh`, `./gradlew test`).
...
2. **The rest of the transforms.** 24 of upstream's 40, and the adaptive curve sampler is now
   written, which was the piece four of them shared. `density`, `kde` and `dotbin` are what is left
   of the statistics: `kde` needs a Gaussian kernel with Scott's bandwidth rule, `density` needs the
   four distributions (`normal`, `lognormal`, `uniform`, `kde`) and their mixture, and `dotbin`
   needs Wilkinson's dot-stacking with its variance-reducing swap pass. None needs new machinery.
   The other family is `nest`, `stratify`, `treemap`, `pack`, `partition` and `tree`, which does:
   a tree structure the dataflow has no representation for.
3. **Keep growing the fixture corpus.** 52 of the brief's 100. The return has fallen off — of the
