...
Other packages' vectors on disk predate the recorder fix. They are gitignored and regenerated by
`scripts/record-upstream-vectors.sh`, so the next full recording picks it up; only a function that
mutates an argument is affected.
