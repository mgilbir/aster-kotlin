...
| Core portable to Kotlin Multiplatform | Partial | `NoAndroidTypesTest` | No JVM-only APIs except fixed-precision decimal formatting in three files, which common Kotlin has no equivalent of; the guard test lists those three by name and fails on anything new. Calendar arithmetic goes through `kotlinx-datetime` rather than `java.time` or `Calendar` | — |
