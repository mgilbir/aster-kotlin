...
| `domainMin`, `domainMax`, `domainMid`, `domainRaw`, `domainImplicit`, `bins` | Not implemented | `UnhandledPropertiesTest` | Six of upstream's 23 scale properties, each reported by name. `domainMin`/`domainMax` are the ones in wide use: without them a scale starts and ends wherever its data does | 6 |
