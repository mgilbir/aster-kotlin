...
Upstream Vega has 40 transforms; the 12 the brief lists for the first release are implemented, plus `timeunit`, `pie` and `window`.
...
| The other 25 transforms | Not implemented | `TransformReferenceTest` | An unimplemented transform stops the pipeline and reports it, so later stages never run on data they were not meant to see | 4+ |
...
| Other axis properties | Not implemented | `AxisStyleTest`, `UnhandledPropertiesTest` | Upstream's axis takes 74 properties. The forty-odd this engine does not honour — `labelLimit`, `labelBound`, `labelFlush`, `tickMinStep`, `tickBand`, `tickExtra`, `minExtent`, `titleAngle`, the `*Cap` and `*DashOffset` family and the rest — are each reported by name with what will be drawn instead | 6 |
