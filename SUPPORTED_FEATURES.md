...
| Line | **Supported** | `FixtureDifferentialTest` (`line-area` fixture) | One `PathNode` per series where upstream emits one item per datum; the harness normalizes both to a point list. A gap breaks the line, as `defined` does. Interpolation methods other than linear are reported, not approximated | 3 |
| Area | **Supported** | `FixtureDifferentialTest` (`line-area` fixture) | Both orientations, via the (x, y) and (x2, y2) boundary pairs. Stacking comes from the `stack` transform. Interpolation as for `line` | 3 |
...
| Axes | **Supported** | `SpecCompilerTest`, `FixtureDifferentialTest` | Ticks, labels, gridlines, domain line and title for all four orientations. Tick positions, label anchors, alignment and text match upstream exactly. No label overlap removal, no `encode` overrides, no format strings | 5 |
