...
| Series normalization | Supported | `Differential`, `oracle-js/src/normalize.js` | Vega emits one item per datum for a line or area; this engine builds one path. Both sides collapse to an outline point list, compared numerically. A **step** interpolation puts corners between the data points, so the item list is no longer the outline — the normalizer asks `d3-shape` itself for the corners rather than reconstructing them, through a recording context because `d3-path` rounds its string output to three decimals. The interpolation method is compared alongside the points, since three different curves can pass through the same ones | 3 |
