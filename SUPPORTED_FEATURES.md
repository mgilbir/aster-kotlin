...
| Other axis properties | Not implemented | `AxisStyleTest` | Upstream's axis takes 74 properties. The fifty-odd this engine does not honour — `values`, `labelAngle`, `labelOverlap`, `labelLimit`, `tickMinStep`, `tickBand`, `minExtent`, `titleAngle`, the `*Cap` and `*DashOffset` family and the rest — are each reported by name with what will be drawn instead. `values` gets the strongest wording: an axis that quietly draws the scale's own ticks where four were named is not a partly-supported axis, it is a different one | 6 |
